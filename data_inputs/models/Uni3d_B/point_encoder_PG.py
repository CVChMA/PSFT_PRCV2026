...
class PointcloudEncoder(nn.Module):
    def __init__(self, point_transformer, config, args):
        super().__init__()
        self.trans_dim = config.trans_dim
        self.embed_dim = config.embed_dim
        self.group_size = config.group_size
        self.num_group = config.num_group
        self.encoder_dim = config.encoder_dims
        self.nb_cls = args.nb_cls
        # # setting a patch_dropout of 0. would mean it is disabled and this function would be the identity fn
        self.patch_dropout = PatchDropout(config.patch_dropout) if config.patch_dropout else nn.Identity()
        ...
        self.cls_head_finetune = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear(256, self.nb_cls)
        )

    def freeze(self):
        for n, p in self.named_parameters():
            p.requires_grad = False
            if 'prompt' in n or 'cls' in n or 'pos' in n or 'adapt_point' in n or 'norm' in n or 'trans2embed' in n:
    ...
    def forward(self, pts, colors):
        ...
        prompt_f = prompt_f.permute(0, 2, 1)
        ...
        # print(x.shape)
        # print(prompt_f.shape)
        

        # ModuleList not support forward
        for i, blk in enumerate(self.visual.blocks):
            x = torch.concat([x, prompt_f], dim=1)
            # print(x.shape)
            x = blk(x)
        # import sys
        # sys.exit(1)
        token = x.max(1)[0] + x.mean(1)

        x = self.visual.norm(token) 
        x = self.visual.fc_norm(x)

        x = self.trans2embed(x)
    
        ret = self.cls_head_finetune(x)
